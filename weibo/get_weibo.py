...
Cookie = "SUB=_2A25JkHgfDeRhGedJ61UR9y3LzjiIHXVrexhXrDV6PUJbktAGLWOnkW1NVn_TfaGxjLW82NnQCI06QcT9icv53Ebo; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9W5an-bDDKgzfrpVhr.4_rij5NHD95QpS05NehM0S0-XWs4DqcjZwGxydJURUs8V97tt; SSOLoginState=1687423054; _T_WM=81592146171; WEIBOCN_FROM=1110006030; MLOGIN=1; M_WEIBOCN_PARAMS=luicode%3D20000174%26uicode%3D20000174"
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                if endtime > weibo['publish_time']:
                    if  not weibo['content'].startswith('[置顶]'): 
                        print(f"{endtime} > " + weibo['publish_time'] + ", exit!")
                        return -1
            else:
                weibo = None
            return weibo
        ...
        try:
            ...
            if is_exist:
                for i in range(0, len(info) - 1):
                    ...
                    print(f"get weibo return: {weibo}")
                    if weibo == -1:
                        print(f"get weibo return -1")
                        return -1                          
                    elif weibo:
...
def scrape_wu2198():
    ...
    table.set_index('微博id',inplace=True)      
    wu2198_csv = "user/wu2198.csv"
    wu2198_table = pd.DataFrame(columns=column_names)
